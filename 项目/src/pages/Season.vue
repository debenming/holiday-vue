...
export default {
    data() {
        return {
            ...
            message2: {},
            shoppingNum: 1,
            url: [require('../assets/season/1.jpg'),
            require('../assets/season/2.jpg'),
            require('../assets/season/3.jpg'),
            require('../assets/season/4.jpg'),]
        }
    },
    methods: {
        open(m) {
    ...
    created() {
        ...
        request({
            ...
            this.message = this.message.filter((p) => { return p.sellname.indexOf('限定') !== -1 })
        }).catch(err => {
            console.log('查询失败')
            console.log(err);
        })
